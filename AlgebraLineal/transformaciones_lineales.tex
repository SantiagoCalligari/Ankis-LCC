...
\begin{tikzpicture}[remember picture, overlay]
    ...
    { % Content for R3, C1
      $DimV = n$, una base ordenada es un subconjunto $B = \{v_1,...,v_n\}, v \in V: v = \alpha_1 v_1 + ... + \alpha_n v_n$. El vector $(\alpha_1,...,\alpha_n) \in F$ es el vector de coordenadas v, $[v]_{B}$
      \footnotesize
      \begin{itemize}
        \item V,W F-ev isomorfos si $\exists T:V\rightarrow W$ isomorfo, $V \stackrel{T}{\simeq} W$
          \vspace{-0.5mm}
        \item En la clase de los F-ev, el isomorfismo es relacion de equivalencia. [Reflexiva, Transitiva, Simetrica]
          \vspace{-0.5mm}
        \item V F-ev con $dimV=n \Rightarrow V \simeq F^n$ 
          \vspace{-0.5mm}
        \item $B_1 = \{v_1, ...,v_n\}, B_2=\{w_1,...,w_n\}$, $C = (C_{ij})$/\\$ \forall j < n, c_{1j},...c_{nj} \in F$ /   $v_j = c_{1j}w_1 + ... + c_{nj}w_n$
          \vspace{-0.5mm}
          $C[v]_{B_1}^t = [v]_{B_2}^t$ Cambio de base de $B_1$ a $B_2$
          \vspace{-0.5mm}
        \item $C_{B_1\rightarrow B_3} = C_{B_2 \rightarrow B_3}C_{B_1\rightarrow B_2}$
          \vspace{-0.5mm}
        \item $C_{B_1\rightarrow B_2} = C_{B_2 \rightarrow B_1}^{-1}$
          \vspace{-0.5mm}
        \item Si $A\in F^{nxn}$ invertible $\exists B_1, B_2 de F^n / A = C_{B_1B_2}$
          \vspace{-0.5mm}
        \item Si $A\in F^{nxn}$ invertible Y $B$ base de $F^n \Rightarrow$
          \vspace{-0.5mm}
          \subitem $\exists B_1$ base de $F^n / A = C_{B_1B}$
          \subitem $\exists B_2$ base de $F^n / A = C_{BB_2}$
        \end{itemize}
    };
    ...
    { % Content for R4, C2 (Bottom-Right)
      Dado $B_1=\{e1,e_2,e_3\}$ y $B_2=\{(1,1,1),(1,1,0),(1,0,0)\}$ se consigue la matriz cambio de base $C_{B_1 B_2}$ expresando cada vector de $B_1$ en la base $B_2$.
      $$
      C_{B_1 B_2} = ([e_1]_{B_2}^t, [e_1]_{B_2}^t, [e_1]_{B_2}^t) =
      \begin{pmatrix} 
        0 & 0 & 1 \\ 
        0 & 1 & -1 \\ 
        1 & -1 & 0 \\
      ...
      $$
      \\
      La matriz de T respecto a las bases $B_1, B_2$ está dada por:
    $$
      [T]_{B_1 B_2} = ( [T(v_1)]_{B_2} ... [T(v_n)]_{B_2} )
    $$
      \begin{itemize}
        \item $[T]_{B_2 B_2}[v]_{B_1}^t = [T(v)]_{B_2}^t$
      \end{itemize}
    };
